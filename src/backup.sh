...
if [ -S "$DOCKER_SOCK" ]; then
  ...
  docker ps \
    ...
    > "$TEMPFILE"
  while read line; do
    if [ "$PRE_POST_USER" != "0" ]; then
      info "Pre-exec command: docker exec -u $PRE_POST_USER $line"
      docker exec -u $PRE_POST_USER $line
    else
...
if [ -S "$DOCKER_SOCK" ]; then
  ...
  while read line; do
    if [ "$PRE_POST_USER" != "0" ]; then
      info "Post-exec command: docker exec -u $PRE_POST_USER $line"
      docker exec -u $PRE_POST_USER $line
    else
...
if [ "$BACKUP_MAX_NUMBERS" != "0" ]; then
  echo "Remove Files:"
  find ${BACKUP_ARCHIVE} -type f | sort -r | tail -n +${BACKUP_MAX_NUMBERS}
  find ${BACKUP_ARCHIVE} -type f | sort -r | tail -n +${BACKUP_MAX_NUMBERS} | xargs rm -rf
  echo .
  echo .
fi
...
curl -X POST -H "Content-Type: application/json" -d '{ "success": "true", "time": $TIME_FINISH}' http://localhost:8452/setData
